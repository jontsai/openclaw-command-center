...
const { appendEntry: feedAppend, readEntries: feedRead } = require("./feed");
    ...
    if (req.method === "GET") {
      ...
      req.on("end", () => {
        try {
          ...
          const record = feedAppend(entry);
          // Broadcast to SSE clients
          broadcastSSE("feed", record);
          // Trigger notifications for critical events
          try {
            const { notify } = require("./notifier");
            notify(record);
          } catch (_e) {
            // Notifier not available — non-fatal
          }
