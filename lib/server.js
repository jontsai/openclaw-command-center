      ...
      try {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            // Pattern: "sender": "612312583008944133" and "label": "HarrYahu"
...
async function refreshVitalsAsync() {
  ...
  try {
    ...
    if (isLinux) {
      ...
      const buffersMatch = memInfoRaw.match(/Buffers:\s+(\d+)/);
      const cachedMatch = memInfoRaw.match(/Cached:\s+(\d+)/);
      
      vitals.memory.total = memTotalKB * 1024;
      const memAvailable = parseInt(memAvailableMatch?.[1] || memFreeMatch?.[1] || 0, 10) * 1024;
      const buffers = parseInt(buffersMatch?.[1] || 0, 10) * 1024;
      const cached = parseInt(cachedMatch?.[1] || 0, 10) * 1024;
      ...
      vitals.memory.percent = vitals.memory.total > 0 
      ...
      const wiredPages = parseInt((memInfoRaw.match(/Pages wired down:\s+(\d+)/) || [])[1] || 0, 10);
    ...
    const iostatCmd = isLinux ? "iostat -d 2 2>/dev/null | tail -1 || echo ''" : "iostat -d -c 2 2>/dev/null | tail -1 || echo ''";
    ...
    if (isLinux) {
      const cpuBrand = await runCmd("cat /proc/cpuinfo | grep 'model name' | head -1 | cut -d: -f2", { fallback: "" });
...
function getSystemVitalsOLD_DISABLED() {
  ...
  try {
    ...
    if (isLinux) {
      // Linux: try multiple sources for temperature
      try {
        // Try lm-sensors first
        const sensorsOut = execSync("sensors -j 2>/dev/null || sensors 2>/dev/null", { encoding: "utf8", timeout: 5000 });
        
        // Parse JSON output from sensors -j
        if (sensorsOut.startsWith("{")) {
          const sensorsData = JSON.parse(sensorsOut);
          for (const chip of Object.values(sensorsData)) {
            for (const feature of Object.values(chip)) {
              for (const [key, value] of Object.entries(feature)) {
                if (key.toLowerCase().includes("temp") && typeof value === "number") {
                  vitals.temperature = Math.round(value);
                  break;
                }
              }
          ...
          // Parse text output
          const tempMatch = sensorsOut.match(/(?:Core|Package|Tdie|temp\d+):\s*\+?([\d.]+)/);
          if (tempMatch) {
            vitals.temperature = Math.round(parseFloat(tempMatch[1]));
          }
        }
      } catch (e) {}

      // Fallback: try /sys/class/hwmon
        ...
        try {
          ...
          for (const dir of hwmonDirs) {
            ...
            if (fs.existsSync(tempPath)) {
              ...
              vitals.temperature = Math.round(parseInt(tempRaw, 10) / 1000);
              break;
            }
          }
        } catch (e) {}
        ...
        try {
          ...
          for (const zone of thermalZones) {
            ...
            if (fs.existsSync(tempPath)) {
              ...
              if (temp > 0 && temp < 150000) { // Sanity check
                vitals.temperature = Math.round(temp / 1000);
                break;
              }
            }
          }
      ...
      if (!vitals.temperature) {
        vitals.temperatureNote = "No thermal sensors detected";
      }
    } else {
