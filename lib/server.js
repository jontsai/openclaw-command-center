...
function getCostBreakdown() {
  ...
  const windowConfigs = {
    ...
    '3d': { days: 3, label: '3-day avg' },
    '7d': { days: 7, label: '7-day avg' },
  };
...
function getTokenStats(sessions, capacity) {
  ...
  // Calculate savings for all windows (24h, 3da, 7da)
  const windowConfigs = {
    '24h': { days: 1, label: '24h' },
    '3da': { days: 3, label: '3da' },
    '7da': { days: 7, label: '7da' },
  };
  
  const savingsWindows = {};
  for (const [key, config] of Object.entries(windowConfigs)) {
    // Map '3da' -> '3d' for bucket lookup
    const bucketKey = key.replace('da', 'd').replace('24h', '24h');
