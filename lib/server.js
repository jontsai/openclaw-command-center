...
// Token usage cache
let tokenUsageCache = { data: null, timestamp: 0 };
const TOKEN_USAGE_CACHE_TTL = 30000; // 30 seconds

function getDailyTokenUsage() {
  // Return cached data if fresh
  const now = Date.now();
  if (tokenUsageCache.data && (now - tokenUsageCache.timestamp) < TOKEN_USAGE_CACHE_TTL) {
  ...
  try {
    ...
    // Cache the result
    tokenUsageCache = { data: result, timestamp: Date.now() };
    console.log(`[Token Usage] Cached: ${requestCount} requests, ${totalInput + totalOutput} tokens`);
    
    return result;
  } catch (e) {
    console.error("Failed to aggregate daily usage:", e.message);
    return tokenUsageCache.data; // Return stale cache on error
  }
}
...
server.listen(PORT, () => {
  ...
  setTimeout(async () => {
    ...
    try {
      // Prime the sessions cache first (async, doesn't block)
  ...
  // Background sessions cache refresh every 10 seconds (async)
  setInterval(() => {
    refreshSessionsCache();
  }, SESSIONS_CACHE_TTL);
});
