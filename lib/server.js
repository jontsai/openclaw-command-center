...
const STATE_CACHE_TTL = 10000; // 10 seconds (longer cache = less CLI calls)
...
function getFullState() {
  ...
  // Skip CLI-based calls in state refresh - they block and are often broken
  // Sessions and subagents use OpenClaw CLI which can hang
  // try { sessions = getSessions(); } catch (e) { console.error("[State] sessions:", e.message); }
  try { tokenStats = getTokenStats(sessions); } catch (e) { console.error("[State] tokenStats:", e.message); }
  try {
    statusCounts = {
      all: sessions.length,
      live: sessions.filter((s) => s.active).length,
  ...
  // Skip CLI-based capacity call - blocks server
  ...
  // Skip CLI-based subagents call - blocks server
  // try { subagents = getSubagentStatus(); } catch (e) { console.error("[State] subagents:", e.message); }
...
function getTokenStats(sessions) {
  ...
  try {
    const configOutput = execSync("cat ~/.openclaw/openclaw.json 2>/dev/null || cat ~/.openclaw/config.json 2>/dev/null", {
      encoding: "utf8",
    });
