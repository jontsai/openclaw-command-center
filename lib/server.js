  ...
  try {
    ...
    if (jsonStr) {
      ...
      const mapped = sessions.map((s) => {
        ...
        // Estimate session age (assume session started when first tokens appeared)
        // For active sessions, calculate burn rate = tokens / session_age_minutes
        // Using a rough heuristic: sessions older than 24h use 24h as max age
        // Min 1 minute to avoid division by tiny numbers
        const sessionAgeMinutes = Math.max(1, Math.min(minutesAgo, 24 * 60)); // 1 min to 24h
        ...
        return {
          ...
          metrics: {
            burnRate: burnRate,
            toolCalls: s.toolCalls || 0,  // TODO: parse transcripts for accurate count
            minutesActive: Math.max(1, Math.min(Math.round(minutesAgo), 24 * 60)),  // 1 min to 24h
          },
        };
      });
...
function getDailyTokenUsage() {
  try {
    ...
    for (const file of files) {
      ...
      // Skip files not modified in the last 7 days (optimization - old files won't have recent entries)
      if (stat.mtimeMs < now - 7 * 24 * 60 * 60 * 1000) continue;
      ...
      try {
        ...
        for (const line of lines) {
          ...
          try {
            const entry = JSON.parse(line);
            // Filter by ENTRY timestamp, not file mtime
...
function getCostBreakdown() {
  ...
  // Calculate costs
  const inputCost = (daily.input / 1_000_000) * rates.input;
  const outputCost = (daily.output / 1_000_000) * rates.output;
  const cacheReadCost = (daily.cacheRead / 1_000_000) * rates.cacheRead;
  const cacheWriteCost = (daily.cacheWrite / 1_000_000) * rates.cacheWrite;
  ...
  return {
    // Raw token counts
    inputTokens: daily.input,
    outputTokens: daily.output,
    cacheRead: daily.cacheRead,
...
function getTokenStats(sessions, capacity) {
  ...
  // Calculate cost consistently with getCostBreakdown (includes cache costs)
  // Claude Opus 4 pricing (per 1M tokens)
  const rates = { input: 15.00, output: 75.00, cacheRead: 1.50, cacheWrite: 18.75 };
  const inputCost = (totalInput / 1_000_000) * rates.input;
  const outputCost = (totalOutput / 1_000_000) * rates.output;
  const cacheReadCost = ((daily?.cacheRead || 0) / 1_000_000) * rates.cacheRead;
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === "/api/status") {
    ...
    try {
      ...
      sendSSE(res, "update", state);
    } catch (e) {
      console.error("[SSE] Initial state error:", e.message);
    }
...
// SSE heartbeat - broadcast full state every 30 seconds
setInterval(() => {
...
}, 5000); // 5-second updates (balanced between responsive and not overwhelming)
