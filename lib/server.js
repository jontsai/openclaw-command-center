...
function getLlmUsage() {
  ...
  try {
    if (fs.existsSync(stateFile)) {
      ...
      // Only use file data if it has valid usage values (not 0 or undefined)
      const hasValidSession = data.claude?.session?.used_pct !== undefined && data.claude?.session?.used_pct !== null;
      const hasValidWeekly = data.claude?.weekly_all_models?.used_pct !== undefined;
      if (hasValidSession || hasValidWeekly) {
        return {
          timestamp: new Date().toISOString(),
          source: "file",
          claude: {
