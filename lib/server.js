...
async function refreshVitalsAsync() {
  ...
  try {
    ...
    // NOTE: iostat needs an explicit count, otherwise it runs forever on Linux.
    // We sample 2 iterations and take the last line.
    const iostatCmd = isLinux
      ? "iostat -d 2 2 2>/dev/null | tail -1 || echo ''"
...
function getSystemVitalsOLD_DISABLED() {
  ...
  try {
    ...
    try {
      ...
      const iostatRaw = execSync("iostat -d -c 2 2 2>/dev/null | tail -1", { encoding: "utf8" });
...
function getFullState() {
  ...
  // IMPORTANT: use ALL sessions, not just the first page slice.
  try {
    tokenStats = getTokenStats(allSessions, capacity);
  } catch (e) {
